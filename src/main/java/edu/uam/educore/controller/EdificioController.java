...
public class EdificioController {
  ...
  public Aula agregarAula(int edificioId, String codigo, int capacidad, TipoAula tipo)
      throws Exception {
    ...
    validarCodigoAulaDuplicado(edificio, codigo, 0);

    Aula aula = new Aula(0, codigo, capacidad, tipo, edificio);
  ...
  public Aula actualizarAula(
      int edificioId, int aulaId, String codigo, int capacidad, TipoAula tipo) throws Exception {
    ...
    Aula aulaEncontrada = buscarAulaEnEdificio(edificio, aulaId);

    if (aulaEncontrada == null) {
      throw new IllegalArgumentException("El aula no pertenece al edificio indicado.");
    }

    validarAula(codigo, capacidad, tipo);

    validarCodigoAulaDuplicado(edificio, codigo, aulaId);
  ...
  private Aula buscarAulaEnEdificio(Edificio edificio, int aulaId) {
  ...
  public void eliminarAula(int edificioId, int aulaId) throws Exception {
    ...
    if (!(repo instanceof EdificioRepoSql repoSql)) {
      throw new IllegalStateException("El repositorio actual no permite eliminar aulas.");
    }
  ...
  private void validarAula(String codigo, int capacidad, TipoAula tipo) {
  ...
  private void validarCodigoAulaDuplicado(Edificio edificio, String codigo, int aulaIdActual) {

    for (Aula aula : edificio.getAulas()) {

      boolean mismoCodigo = aula.getCodigo().equalsIgnoreCase(codigo.trim());

      boolean aulaDiferente = aula.getId() != aulaIdActual;

      if (mismoCodigo && aulaDiferente) {
        throw new IllegalArgumentException(
            "Ya existe un aula con ese código " + "dentro del edificio.");
      }
    }
  }
}
