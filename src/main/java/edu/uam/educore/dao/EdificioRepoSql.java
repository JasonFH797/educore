...
public class EdificioRepoSql extends Repositorio<Edificio> {
  ...
  private void guardarAulas(Connection con, Edificio edificio) throws Exception {
    String sql = "INSERT INTO aula (codigo, capacidad, tipo, edificio_id) " + "VALUES (?, ?, ?, ?)";
    ...
    try (Connection con = abrir()) {
      ...
      try {
        ...
        try (PreparedStatement ps = con.prepareStatement(sqlEdificio)) {
          ...
          if (filas == 0) {
            throw new IllegalArgumentException("No existe el edificio que se desea actualizar.");
          }
        }
  ...
  private void insertarAula(Connection con, Edificio edificio, Aula aula) throws Exception {
    String sql =
        "INSERT INTO aula " + "(codigo, capacidad, tipo, edificio_id) " + "VALUES (?, ?, ?, ?)";
  ...
  private void actualizarAula(Connection con, Edificio edificio, Aula aula) throws Exception {
    String sql =
        "UPDATE aula "
    ...
    try (PreparedStatement ps = con.prepareStatement(sql)) {
      ...
      if (filas == 0) {
        throw new IllegalArgumentException("No existe el aula que se desea actualizar.");
      }
    }
  }
  ...
  public void eliminar(int id) throws Exception {
    ...
    try (Connection con = abrir();
      ...
      if (filasAfectadas == 0) {
        throw new IllegalArgumentException("No existe un edificio con el ID indicado.");
      }
    }
  }
  ...
  private Aula mapearAula(ResultSet rs, Edificio edificio) throws Exception {
    ...
    return new Aula(id, codigo, capacidad, tipo, edificio);
  }
